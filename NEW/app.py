...
@app.route("/user_queries/<int:page>")
def user_queries(page=1):
    s_id = str(session['id'])
    perpage = 4
    startat = (page * perpage) - perpage
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        'select LTRIM(query.id) as id,CAST(query.Date as DATE) as d, query.keyword, query.status from query where query.user_id= %s order by date desc',
            [s_id])
    ...
    if request.method == "POST":
        ...
        return render_template('user-queries.html', plans=Data , page=page, total=final,
